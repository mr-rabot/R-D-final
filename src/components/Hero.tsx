...
export function Hero({ initialData }: HeroProps) {
  ...
  return (
    <section className="hero-gradient text-white pt-32 pb-20 md:pt-48 md:pb-32 lg:pb-40 overflow-hidden relative min-h-[90vh] flex items-center w-full">
          ...
          {isLoading ? (
            <div className="space-y-10 w-full">
              ...
              <div className="space-y-4">
                ...
                <Skeleton className="h-16 w-4/5 bg-white/10 rounded-2xl" />
              </div>
            ...
            <div className="space-y-10 md:space-y-12 text-center lg:text-left animate-in fade-in slide-in-from-bottom-12 duration-1000 w-full">
              ...
              <h1 className="text-4xl sm:text-6xl md:text-7xl lg:text-8xl font-headline font-bold leading-[1.1] tracking-tight">
              ...
              <p className="text-lg md:text-xl lg:text-2xl text-blue-100/85 max-w-2xl leading-relaxed font-light italic mx-auto lg:mx-0 border-l-4 border-white/10 pl-6 md:pl-8">
              ...
              <div className="flex flex-col sm:flex-row items-center justify-center lg:justify-start gap-6 pt-6">
                <Button 
                  ...
                  size="lg" 
                  className="w-full sm:w-auto rounded-[20px] px-10 h-16 text-lg bg-white text-primary hover:bg-blue-50 shadow-xl font-bold flex gap-3 transition-all active:scale-95"
                >
                ...
                <Button 
                  ...
                  variant="outline" 
                  size="lg" 
                  className="w-full sm:w-auto rounded-[20px] px-10 h-16 text-lg border-2 border-white/40 text-white hover:bg-white/10 font-bold bg-transparent flex gap-3 backdrop-blur-md transition-all active:scale-95"
                >
                  <Mail className="h-5 w-5" /> Email Desk
                </Button>
              </div>

              <div className="grid grid-cols-2 sm:grid-cols-3 gap-8 md:gap-12 pt-16 border-t border-white/15 text-center sm:text-left">
                {(heroData?.stats || [
                  ...
                  <div key={i} className="space-y-2">
                    <div className="text-4xl md:text-5xl font-headline font-bold leading-none">{stat.value}</div>
                    <div className="text-[10px] md:text-xs text-blue-200/60 uppercase tracking-[0.3em] font-bold">{stat.label}</div>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="mt-20 lg:mt-0 relative flex justify-center w-full animate-in fade-in zoom-in duration-1000 delay-500">
            <div className="relative z-10 w-full max-w-lg">
              {isLoading ? (
                ...
                heroData?.image && (
                  <div className="relative aspect-[4/5] w-full overflow-hidden rounded-[40px] md:rounded-[60px] border-8 border-white/5 shadow-2xl group">
                    <Image
                      src={heroData.image}
                      alt="Hero Visual"
                      fill

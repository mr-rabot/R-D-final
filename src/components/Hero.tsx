      ...
      <div className="w-full max-w-[1440px] mx-auto px-6 md:px-12 lg:px-24 relative z-10">
        <div className="flex flex-col lg:grid lg:grid-cols-2 lg:gap-20 items-center">
          {isLoading ? (
            ...
            <div className="space-y-8 md:space-y-12 text-center lg:text-left animate-in fade-in slide-in-from-bottom-8 duration-1000 w-full">
              <div className="inline-flex items-center gap-2 bg-white/10 backdrop-blur-md border border-white/20 px-5 py-2 rounded-full text-white font-bold text-[10px] md:text-xs uppercase tracking-widest mx-auto lg:mx-0 shadow-xl">
                <Sparkles className="h-4 w-4 text-blue-300" />
                {heroData?.badge || "Premier Research Excellence"}
              </div>
              
              <h1 className="text-4xl sm:text-6xl md:text-7xl lg:text-8xl xl:text-9xl font-headline font-bold leading-[1.1] tracking-tight">
              ...
              <p className="text-lg md:text-2xl text-blue-100/80 max-w-2xl leading-relaxed font-light mx-auto lg:mx-0">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-6 md:gap-10 pt-12 md:pt-16 border-t border-white/10 text-center sm:text-left">
                {(heroData?.stats || [
                  { label: "Papers Published", value: "500+" },
                  { label: "Acceptance Rate", value: "98%" },
                  { label: "Expert Concierge", value: "24/7" }
                ]).map((stat: any, i: number) => (
                  <div key={i} className="space-y-1 md:space-y-2 group">
                    <div className="text-3xl md:text-5xl lg:text-6xl font-headline font-bold group-hover:text-blue-400 transition-colors">{stat.value}</div>
                    <div className="text-[8px] md:text-[10px] text-blue-200/60 uppercase tracking-[0.2em] font-bold">{stat.label}</div>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="mt-16 lg:mt-0 relative flex justify-center w-full lg:block animate-in fade-in zoom-in duration-1000 delay-300">
            <div className="relative z-10 w-full max-w-lg lg:max-w-none">
              {isLoading ? (
                ...
                heroData?.image && (
                  <div className="relative aspect-[4/5] w-full overflow-hidden rounded-[40px] md:rounded-[60px] border-[8px] md:border-[16px] border-white/5 shadow-[0_40px_80px_rgba(0,0,0,0.5)] group">
                    <Image
                      src={heroData.image}
                      alt="Hero Visual"
                      fill
